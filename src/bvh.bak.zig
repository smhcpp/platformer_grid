...
pub const BVH = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !*BVH {
        ...
        var indices: [bvh.platforms.len]usize = undefined;
    ...
    fn buildRecursive(bvh: *BVH, indices: []usize) std.mem.Allocator.Error!*TreeNode {
        ...
        node.left = try bvh.buildRecursive(bvh.platforms, left_indices);
        node.right = try bvh.buildRecursive(bvh.platforms, right_indices);
